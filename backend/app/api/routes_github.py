...
from app.models.schemas import GithubConnectionOut, GithubConnectUrlOut, GithubRepoOut, RepoIngestJobOut
from app.services import github_service
from app.utils.auth_utils import create_access_token, decode_token, get_current_user
...
def connect(current_user: User = Depends(get_current_user)):
    ...
    # Reuse the user's own signed JWT as the OAuth `state` param so the stateless
    # callback (no cookie session) can identify which user is connecting.
    ...
    return GithubConnectUrlOut(authorize_url=f"https://github.com/login/oauth/authorize?{query}")
...
def callback(code: str, state: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return RedirectResponse(f"{app_settings.FRONTEND_ORIGIN}/settings?github_error={exc}")
    ...
    return RedirectResponse(f"{app_settings.FRONTEND_ORIGIN}/settings?github=connected")
...
def get_connection(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return GithubConnectionOut(
        connected=True, github_login=conn.github_login, connected_at=conn.connected_at.isoformat()
    )
...
@router.post("/repos/{owner}/{repo}/ingest", response_model=RepoIngestJobOut)
def ingest_repo(
    owner: str,
    repo: str,
    ...
    conn = _get_connection(db, current_user.id)
    repos = {r["full_name"]: r for r in github_service.list_user_repos(conn.access_token)}
    full_name = f"{owner}/{repo}"
    repo_meta = repos.get(full_name)
    if not repo_meta:
        raise HTTPException(status_code=404, detail="Repository not found or not accessible")

    job = RepoIngestJob(user_id=current_user.id, repo_full_name=full_name, status="pending")
    db.add(job)
    db.commit()
    db.refresh(job)

    background_tasks.add_task(
        github_service.ingest_repo, job.id, conn.access_token, owner, repo, repo_meta["default_branch"]
    )

    return RepoIngestJobOut(
        id=job.id, repo_full_name=job.repo_full_name, status=job.status,
        total_files=job.total_files, processed_files=job.processed_files,
...
def get_job(job_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    job = db.query(RepoIngestJob).filter(RepoIngestJob.id == job_id, RepoIngestJob.user_id == current_user.id).first()
    if not job:
